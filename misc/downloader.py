...
def collect_and_save_proxies():
    ...
    log("run", f"Collecting raw proxy list, targeting {desired_count} active proxies...")
    proxy_sources = [
    ...
    for source in proxy_sources:
        try:
            ...
            log("warn", f"Failed to fetch proxies from {source.split('/')[2]}: {e}")

    if not initial_proxies:
        log("error", "Failed to collect proxies from all sources.")
        return False

    log("info", f"Collected {len(initial_proxies)} raw proxies. Verifying until {desired_count} active proxies are found...")
    ...
    if active_proxies:
        ...
        with open(PROXY_FILE_PATH, 'w', encoding='utf-8') as f:
            for proxy in active_proxies:
                f.write(proxy + '\n')
        log("success", f"Successfully saved {len(active_proxies)} active proxies to {os.path.basename(PROXY_FILE_PATH)}.")
        return True
    else:
